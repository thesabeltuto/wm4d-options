...
function flipper_get_settings() {
	$mode_selected = get_option('wm4d_flipper_server_mode');
	if( is_array($mode_selected)) $mode_selected = $mode_selected[0]; //array, when saved
//	else $mode_selected = $mode_selected; // string, default

//	print_r(get_option('wm4d_flipper_server_mode'));exit;
	...
	if ($mode_selected=='esi') {
		$opts['server_mode']='esi';
		$opts['client_mode']=false;
		$opts['client_flip']=false;
	} elseif ($mode_selected=='php') {
		$opts['server_mode']='php';
		$opts['client_mode']=false;
		$opts['client_flip']=false;
	}
