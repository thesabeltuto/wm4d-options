...
jQuery(document).ready(function($) {
    ...
    jQuery('input[type="submit"]').on('click', function() {
        remove_on_save();
        jQuery('select.wm4d_campaigns').trigger('click');
    });

    jQuery('input.wm4d_trigger_submit').click(function() {
        jQuery('input[type="submit"]').trigger('click', function() {
        remove_on_save();
        });
        jQuery('select.wm4d_campaigns').trigger('click');
    });
...
function add_referers() {
	...
	//FLIPPER OPTIONS | REFERERS
    jQuery('input.wm4d_referers_add').click(function() {
    //alert('click');
		var tr_max = jQuery('tr[id^="referers_"]').length + 1;
		var tr_wrap = '<tr id="referer_TBA'+tr_max+'">';
		var id = '<td><input type="text" readonly="readonly" name="wm4d_referers_id" value="TBA" size="1"  tabindex="-1"/></td>';
		var referer = '<td><input type="text" name="wm4d_flipper_referers[]"  size="48" /></td>';
		var remove = '<td><input type="button" class="wm4d_remove wm4d_referer_remove" name="wm4d_referer_remove" value="-"  tabindex="-1"/></td>';
		...
		jQuery('table#referer').append(tr_wrap);

        convert_phone_format();
		remove_referers();
	});
 
	//FLIPPER OPTIONS | PRIMARY REFERER
    ...
    //alert('click');
		...
		var phonefrom = '<td><input type="text" readonly="readonly" name="wm4d_phone" value="<?php //echo $wm4d_phone ?>"  tabindex="-1"></td>';
	...
	//FLIPPER OPTIONS | MULTIPLE REFERER
    ...
    //alert('click');
		...
		var phonefrom = '<td><select id="wm4d_multiple_selected_phone" name="wm4d_multiple_selected_phone[]"><?php // echo $select_options ?></select></td>';
...
function remove_referers() {
	...
	//FLIPPER OPTIONS | REFERERS
	jQuery('input.wm4d_referer_remove').click(function() {
        if( jQuery('table#referer tr').length == 2 ) {
            jQuery(this).parent().prev().children().val('');
        }	else { 
            jQuery(this).parent().parent().remove();
        }
...
function remove_on_save() {
	...
	//MULTIPLE REFERER LIST
		jQuery('tr[id^="referer_"]').each(function() {
			var referer = jQuery(this).children().next().children().val();
			var id = jQuery(this).attr('id');
			if(referer == '') {
				jQuery(this).remove();
			}
...
function  update_on_select() {
    jQuery('select.wm4d_campaigns').on('click',function() {
        jQuery('select.wm4d_campaigns.primary_campaign').each( function() {
            var id = jQuery(this).attr('id').substr(4);
            var value = jQuery(this).val(); 
            var hidden = id+':'+value;
            jQuery(this).next().attr('value', hidden);
            });
        jQuery('select.wm4d_campaigns.multiple_campaign').each( function() {
            var id = jQuery(this).attr('id').split('_index_')[0].substr(4);
            var index = jQuery(this).attr('id').split('_index_')[1];
            var value = jQuery(this).val(); 
            var hidden = index+':'+id+':'+value;
            jQuery(this).next().attr('value', hidden);
            });
        });
}